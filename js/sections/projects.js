...
// Maps video index → stable DOM id used for anchor links
...
// ─── Poster placeholder ────────────────────────────────────────────────────────
// Renders a pure-CSS dark card so the video container looks intentional before
// the user clicks play. No external image files needed.
// emoji  — shown large in the centre
// label  — caption below the emoji
// accent — hex colour for the subtle gradient tint (matches the project palette)
function buildPosterHTML(emoji, label, accent = '#10b981') {
  return `
    <div
      class="video-poster absolute inset-0 flex flex-col items-center justify-center select-none pointer-events-none"
      style="
    ...
    >
      ...
      <div style="position:relative; text-align:center;">
        <div style="font-size:3rem; line-height:1; margin-bottom:.5rem; filter:drop-shadow(0 0 12px ${accent}88);">
          ${emoji}
        </div>
        <p style="
          color:#e2e8f0; font-size:.75rem; font-weight:700;
          letter-spacing:.12em; text-transform:uppercase;
          text-shadow: 0 1px 4px rgba(0,0,0,.8);
        ">${label}</p>
      </div>

      <!-- play hint at the bottom -->
      <div style="
        position:absolute; bottom:14px;
        display:flex; align-items:center; gap:6px;
        color:${accent}; font-size:.65rem; font-weight:600; letter-spacing:.08em;
        opacity:.8;
      ">
        <svg width="12" height="12" viewBox="0 0 12 12" fill="${accent}">
...
// Accent colours that match each video's role in the palette
const VIDEO_ACCENTS = {
  'aoun-auth-video': '#8b5cf6', // purple  — security / lock
  'aoun-driver-video': '#3b82f6', // blue    — maps / GPS
  'aoun-student-video': '#10b981', // green   — passenger / student
  'aoun-web-promo': '#06b6d4', // cyan    — web / React
};
...
export default function render(container, data) {
  ...
  const cardsHTML = data.map((p, idx) => {
    ...
    if (baseImg) {
      ...
      const tagsHTML = p.tags ? p.tags.slice(0, 2).map((t, tIdx) => {
        ...
        const badgeColor = isFirst ? (style.text.split(' ')[0] || 'text-brand-600') : 'text-gray-800 dark:text-gray-200';
    ...
    if (p.links.length === 1) {
      ...
      buttonsHTML = `
        <div class="grid grid-cols-2 gap-2 sm:gap-3">
          ...
          <a href="${p.links[1].url}" target="_blank" rel="noopener noreferrer"
    ...
    if (p.featured) {
      ...
      return `
        <div class="lg:col-span-2 group bg-white dark:bg-dark-100 rounded-xl md:rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl ${style.shadow} hover:-translate-y-1 md:hover:-translate-y-2 transition-all duration-300 border border-gray-100 dark:border-dark-200 card-hover"
          data-aos="fade-up" data-aos-duration="600">
          <div class="grid md:grid-cols-2 gap-0">
            <!-- Thumbnail -->
            <div class="relative w-full h-56 sm:h-64 md:h-80 lg:h-full overflow-hidden bg-gray-100 dark:bg-dark-200">
              <img
                srcset="${featImg400} 400w, ${featImg800} 800w, ${featImg1200} 1200w"
            ...
            <div class="p-5 sm:p-6 md:p-8 flex flex-col justify-between">
              <div>
                ...
                <div class="mb-3 sm:mb-4">
                  <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-2">🛠️ TECHNOLOGY STACK</p>
                ...
                <div class="mb-4 sm:mb-6">
                  <p class="text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2 sm:mb-3">📹 Video Demonstrations:</p>
                  <div class="grid grid-cols-2 gap-2 sm:gap-3 text-xs">
                    ${videoLinksHTML}
                  </div>
                </div>
    ...
    // ── Standard card ──────────────────────────────────────────────────────────
    return `
      <div class="group bg-white dark:bg-dark-100 rounded-xl md:rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl ${style.shadow} hover:-translate-y-1 md:hover:-translate-y-2 transition-all duration-300 border border-gray-100 dark:border-dark-200 card-hover"
        data-aos="fade-up" data-aos-delay="${delay}" data-aos-duration="600">
        ${imgHTML}
  ...
  section.innerHTML = `
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <h2 class="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-8 sm:mb-12 md:mb-16 text-center"
        data-aos="fade-up" data-aos-duration="600">
        Featured Projects
      </h2>
  ...
  if (detailProject) renderShowcase(container, detailProject);
}
...
function renderShowcase(container, project) {
  ...
  const highlightIcons = ["fas fa-lock", "fas fa-car", "fas fa-user-graduate", "fas fa-desktop"];
  ...
  const highlightsHTML = details.highlights.map((h, idx) => {
    ...
    const accent = VIDEO_ACCENTS[videoId] || '#10b981';

    // Build the poster: same emoji + label as defined in data.js
    const posterHTML = buildPosterHTML(video.emoji || '📹', video.label, accent);
    ...
    return `
      <div id="${videoId}"
        ...
        <!--
          .video-container  — hook for lazyVideo.js (click-to-load)
          data-video-src    — the MP4 path, loaded only on user click
          .video-poster     — purely visual, hidden once video loads
          .video-placeholder — lazyVideo.js appends the <video> element here
          .play-overlay     — lazyVideo.js controls this (show/hide spinner & play btn)
        -->
        <div
          ...
          role="button"
          tabindex="0"
          aria-label="Click to load and play: ${h.title}"
        >
          <!-- Poster — shown before the user clicks -->
          ${posterHTML}

          <!-- lazyVideo.js appends the real <video> element inside this div -->
          <div class="video-placeholder absolute top-0 left-0 w-full h-full"></div>

          <!-- Play overlay — lazyVideo.js toggles display on this -->
          <div class="play-overlay absolute inset-0 flex items-center justify-center" style="display:flex;">
            <div class="w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center hover:scale-110 transition-transform duration-200"
              style="background:rgba(255,255,255,0.15); backdrop-filter:blur(8px); border:2px solid rgba(255,255,255,0.3); box-shadow:0 0 32px ${accent}55;">
              <svg width="28" height="28" viewBox="0 0 28 28" fill="white" style="margin-left:3px;">
                <polygon points="6,3 25,14 6,25"/>
              </svg>
            </div>
          </div>
        </div>
  ...
  const ecosystemHTML = details.ecosystem.map((e, idx) => {
    const c = ecosystemColors[idx] || ecosystemColors[0];
    return `
      <div class="bg-gradient-to-br ${c.bg} rounded-lg md:rounded-xl p-4 sm:p-6 border ${c.border}"
        data-aos="fade-up" data-aos-delay="${idx * 100}" data-aos-duration="600">
        <div class="flex items-center justify-between mb-3 sm:mb-4">
          <div class="flex items-center gap-2 sm:gap-3">
            <i class="${c.icon} text-xl sm:text-2xl ${c.text}"></i>
            <h4 class="text-base sm:text-lg font-bold text-gray-900 dark:text-white">${e.label}</h4>
          </div>
          <span class="px-2 py-1 ${c.badge} rounded text-xs font-semibold">${e.owner}</span>
        </div>
        <p class="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mb-3 font-semibold">${e.stack}</p>
        <p class="text-xs text-gray-500 dark:text-gray-500">${e.summary}</p>
      </div>
    `;
  }).join('');

  const toolColors = ["text-blue-600 dark:text-blue-400", "text-cyan-600 dark:text-cyan-400", "text-gray-900 dark:text-white", "text-orange-600 dark:text-orange-400", "text-red-600 dark:text-red-400", "text-indigo-600 dark:text-indigo-400"];
  const toolsHTML = details.tools.map((t, idx) => `
    <div class="flex items-start gap-2 sm:gap-3">
      <div class="w-9 h-9 sm:w-10 sm:h-10 bg-gray-100 dark:bg-gray-800 rounded-lg flex items-center justify-center flex-shrink-0">
        <i class="${t.icon} ${toolColors[idx % toolColors.length]} text-sm sm:text-base"></i>
      </div>
      <div class="min-w-0">
        <h4 class="font-semibold text-gray-900 dark:text-white text-xs sm:text-sm truncate">${t.name}</h4>
        <p class="text-xs text-gray-600 dark:text-gray-400">${t.sub}</p>
      </div>
    </div>
  `).join('');

  const leadershipHTML = details.leadership.map(l => `<li class="flex items-start gap-2"><i class="fas fa-chevron-right text-green-500 mt-0.5 text-xs flex-shrink-0"></i><span>${l}</span></li>`).join('');
  const achievementsHTML = details.achievements.map(a => `<li class="flex items-start gap-2"><i class="fas fa-chevron-right text-green-500 mt-0.5 text-xs flex-shrink-0"></i><span>${a}</span></li>`).join('');

  const statColors = ["text-purple-600 dark:text-purple-400", "text-blue-600 dark:text-blue-400", "text-green-600 dark:text-green-400", "text-cyan-600 dark:text-cyan-400"];
  const statsHTML = details.stats.map((s, idx) => `
    <div class="text-center p-3 sm:p-4 md:p-6 bg-white dark:bg-dark-100 rounded-lg md:rounded-xl border border-gray-200 dark:border-dark-200 hover:shadow-lg transition-all duration-300">
      <div class="text-2xl sm:text-3xl md:text-4xl font-bold ${statColors[idx % statColors.length]} mb-1 sm:mb-2">${s.value}</div>
      <p class="text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-300">${s.label}</p>
      <p class="text-xs text-gray-500 dark:text-gray-500 mt-1">${s.sub}</p>
    </div>
  `).join('');
  ...
  showcase.innerHTML = `
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="text-center mb-8 sm:mb-10 md:mb-12">
        ...
        <p class="text-sm sm:text-base md:text-lg text-gray-600 dark:text-gray-400 max-w-3xl mx-auto mb-4 sm:mb-5 md:mb-6 leading-relaxed px-4"
          data-aos="fade-up" data-aos-delay="100" data-aos-duration="600">
          Dual-platform solution combining <strong>React.js web application</strong> for housing &amp; opportunities
          with <strong>Flutter mobile app</strong> for transportation management.
          Powered by <strong>ASP.NET Core</strong> backend.
        </p>
        <div class="inline-flex flex-col sm:flex-row items-center gap-3 sm:gap-4 px-4 sm:px-6 py-4 sm:py-3 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-200 dark:border-green-800 max-w-2xl mx-auto"
      ...
      <div class="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/10 dark:to-indigo-900/10 rounded-xl md:rounded-2xl p-4 sm:p-6 md:p-8 border-2 border-blue-200 dark:border-blue-800 mb-6 sm:mb-8 md:mb-10"
        data-aos="fade-up" data-aos-duration="600">
        <div class="text-center mb-6 sm:mb-8">
          ...
          <h3 class="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">My Technical Contributions</h3>
          <p class="text-sm sm:text-base text-gray-600 dark:text-gray-400 mt-2">${details.myRoleSummary}</p>
        </div>
        <div class="grid md:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
          <div class="bg-white dark:bg-dark-100 rounded-lg md:rounded-xl p-4 sm:p-6 border-2 border-purple-200 dark:border-purple-800 hover:shadow-lg transition-all duration-300">
            ...
            <ul class="space-y-2.5 sm:space-y-3 text-xs sm:text-sm">${backendListHTML}</ul>
          </div>
          <div class="bg-white dark:bg-dark-100 rounded-lg md:rounded-xl p-4 sm:p-6 border-2 border-blue-200 dark:border-blue-800 hover:shadow-lg transition-all duration-300">
            ...
            <ul class="space-y-2.5 sm:space-y-3 text-xs sm:text-sm">${webListHTML}</ul>
          </div>
        </div>
      </div>
      ...
      <div class="mb-6 sm:mb-8 md:mb-10">
        ...
        <div class="grid md:grid-cols-3 gap-4 sm:gap-6">${ecosystemHTML}</div>
      </div>

      <!-- ── Tools ── -->
      <div class="bg-gradient-to-br from-gray-50 to-slate-50 dark:from-gray-900/10 dark:to-slate-900/10 rounded-lg md:rounded-xl p-4 sm:p-6 md:p-8 border border-gray-200 dark:border-gray-800 mb-6 sm:mb-8 md:mb-10"
        data-aos="fade-up" data-aos-duration="600">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-5">${toolsHTML}</div>
      </div>

      <!-- ── Leadership ── -->
      <div class="bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/10 dark:to-emerald-900/10 rounded-lg md:rounded-xl p-4 sm:p-6 md:p-8 border border-green-200 dark:border-green-800 mb-6 sm:mb-8 md:mb-10"
        data-aos="fade-up" data-aos-duration="600">
        ...
        <div class="grid sm:grid-cols-2 gap-4 sm:gap-6">
          <div>
            <h4 class="text-base sm:text-lg font-semibold text-green-700 dark:text-green-400 mb-3 flex items-center gap-2">
              <i class="fas fa-users-cog text-sm sm:text-base"></i><span>Team Leadership</span>
            </h4>
            <ul class="space-y-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400">${leadershipHTML}</ul>
          </div>
          <div>
            <h4 class="text-base sm:text-lg font-semibold text-green-700 dark:text-green-400 mb-3 flex items-center gap-2">
              <i class="fas fa-code text-sm sm:text-base"></i><span>Technical Achievements</span>
            </h4>
            <ul class="space-y-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400">${achievementsHTML}</ul>
          </div>
        </div>
      </div>

      <!-- ── Stats ── -->
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mb-6 sm:mb-8 md:mb-10"
        data-aos="fade-up" data-aos-duration="600">
        ${statsHTML}
      </div>
